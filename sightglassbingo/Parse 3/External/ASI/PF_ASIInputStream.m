...
@implementation PF_ASIInputStream

+ (void)initialize
{
	if (self == [PF_ASIInputStream class]) {
		readLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(PF_ASIHTTPRequest *)theRequest
{
	PF_ASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(PF_ASIHTTPRequest *)theRequest
{
	PF_ASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	...
	if ([PF_ASIHTTPRequest isBandwidthThrottled]) {
		toRead = [PF_ASIHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	[PF_ASIHTTPRequest incrementBandwidthUsedInLastSecond:toRead];
	[readLock unlock];
