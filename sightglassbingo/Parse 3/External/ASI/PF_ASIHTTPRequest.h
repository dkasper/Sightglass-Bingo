...
@class PF_ASIDataDecompressor;

extern NSString *PF_ASIHTTPRequestVersion;
...
typedef enum _PF_ASIAuthenticationState {
	PF_ASINoAuthenticationNeededYet = 0,
	PF_ASIHTTPAuthenticationNeeded = 1,
	PF_ASIProxyAuthenticationNeeded = 2
} PF_ASIAuthenticationState;

typedef enum _PF_ASINetworkErrorType {
    PF_ASIConnectionFailureErrorType = 1,
    PF_ASIRequestTimedOutErrorType = 2,
    PF_ASIAuthenticationErrorType = 3,
    PF_ASIRequestCancelledErrorType = 4,
    PF_ASIUnableToCreateRequestErrorType = 5,
    PF_ASIInternalErrorWhileBuildingRequestType  = 6,
    PF_ASIInternalErrorWhileApplyingCredentialsType  = 7,
	PF_ASIFileManagementError = 8,
	PF_ASITooMuchRedirectionErrorType = 9,
	PF_ASIUnhandledExceptionError = 10,
	PF_ASICompressionError = 11
	
} PF_ASINetworkErrorType;


// The error domain that all errors generated by ASIHTTPRequest use
extern NSString* const PF_NetworkRequestErrorDomain;
...
extern unsigned long const PF_ASIWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^PF_ASIBasicBlock)(void);
typedef void (^PF_ASIHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^PF_ASISizeBlock)(long long size);
typedef void (^PF_ASIProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^PF_ASIDataBlock)(NSData *data);
#endif

@interface PF_ASIHTTPRequest : NSOperation <NSCopying> {
	...
	id <PF_ASIHTTPRequestDelegate> delegate;
	...
	id <PF_ASIHTTPRequestDelegate, PF_ASIProgressDelegate> queue;
	...
	// If error code is = PF_ASIConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	id <PF_ASIProgressDelegate> uploadProgressDelegate;
	...
	id <PF_ASIProgressDelegate> downloadProgressDelegate;
	...
	PF_ASIAuthenticationState authenticationNeeded;
	...
	PF_ASIDataDecompressor *dataDecompressor;
	...
	PF_ASIHTTPRequest *PACFileRequest;
	...
	PF_ASIBasicBlock startedBlock;

	//block to execute when headers are received
	PF_ASIHeadersBlock headersReceivedBlock;

	//block to execute when request completes successfully
	PF_ASIBasicBlock completionBlock;

	//block to execute when request fails
	PF_ASIBasicBlock failureBlock;

	//block for when bytes are received
	PF_ASIProgressBlock bytesReceivedBlock;

	//block for when bytes are sent
	PF_ASIProgressBlock bytesSentBlock;

	//block for when download size is incremented
	PF_ASISizeBlock downloadSizeIncrementedBlock;

	//block for when upload size is incremented
	PF_ASISizeBlock uploadSizeIncrementedBlock;

	//block for handling raw bytes received
	PF_ASIDataBlock dataReceivedBlock;

	//block for handling authentication
	PF_ASIBasicBlock authenticationNeededBlock;

	//block for handling proxy authentication
	PF_ASIBasicBlock proxyAuthenticationNeededBlock;
	
    //block for handling redirections, if you want to
    PF_ASIBasicBlock requestRedirectedBlock;
	#endif
}
...
@property (assign) PF_ASIHTTPRequest *mainRequest;
...
@property (assign, readonly) PF_ASIAuthenticationState authenticationNeeded;
...
@property (assign) id <PF_ASICacheDelegate> downloadCache;
@property (assign) PF_ASICachePolicy cachePolicy;
@property (assign) PF_ASICacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) PF_ASIDataDecompressor *dataDecompressor;
