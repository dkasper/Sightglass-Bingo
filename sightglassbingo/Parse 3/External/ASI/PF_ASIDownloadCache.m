...
#import "PF_ASIDownloadCache.h"
#import "PF_ASIHTTPRequest.h"
#import <CommonCrypto/CommonHMAC.h>

static PF_ASIDownloadCache *sharedCache = nil;

static NSString *sessionCacheFolder = @"SessionStore";
static NSString *permanentCacheFolder = @"PermanentStore";

@interface PF_ASIDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
- (NSString *)pathToFile:(NSString *)file;
@end

@implementation PF_ASIDownloadCache

- (id)init
{
	self = [super init];
	[self setShouldRespectCacheControlHeaders:YES];
	[self setDefaultCachePolicy:PF_ASIUseDefaultCachePolicy];
...
{
	[[self accessLock] lock];
	[self clearCachedResponsesForStoragePolicy:PF_ASICacheForSessionDurationCacheStoragePolicy];
	[storagePath release];
...
- (NSDate *)expiryDateForRequest:(PF_ASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if (maxAge) {
		...
		if (expires) {
			return [PF_ASIHTTPRequest dateFromRFC1123String:expires];
		}
	}
	return nil;
}
...
- (void)storeResponseForRequest:(PF_ASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if ([request error] || ![request responseHeaders] || ([request cachePolicy] & PF_ASIDoNotWriteToCacheCachePolicy)) {
		[[self accessLock] unlock];
		return;
	}
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(PF_ASIHTTPRequest *)request
{
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:([request cacheStoragePolicy] == PF_ASICacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
{
	[[self accessLock] lock];
	PF_ASICachePolicy cp = defaultCachePolicy;
	[[self accessLock] unlock];
	return cp;
}


- (void)setDefaultCachePolicy:(PF_ASICachePolicy)cachePolicy
{
	[[self accessLock] lock];
	if (!cachePolicy) {
		defaultCachePolicy = PF_ASIAskServerIfModifiedWhenStaleCachePolicy;
	}  else {
...
- (void)clearCachedResponsesForStoragePolicy:(PF_ASICacheStoragePolicy)storagePolicy
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:(storagePolicy == PF_ASICacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
- (BOOL)canUseCachedDataForRequest:(PF_ASIHTTPRequest *)request
{
	...
	if ([request cachePolicy] & PF_ASIDoNotReadFromCacheCachePolicy) {
		return NO;

	// If we don't want to load the request whatever happens, always pretend we have cached data even if we don't
	} else if ([request cachePolicy] & PF_ASIDontLoadCachePolicy) {
		return YES;
	}
	...
	if ([request cachePolicy] & PF_ASIOnlyLoadIfNotCachedCachePolicy) {
		return YES;

	// If we want to fallback to the cache after an error
	} else if ([request complete] && [request cachePolicy] & PF_ASIFallbackToCacheIfLoadFailsCachePolicy) {
		return YES;

	// If we have cached data that is current, we can use it
	} else if ([request cachePolicy] & PF_ASIAskServerIfModifiedWhenStaleCachePolicy) {
	...
	} else if ([request cachePolicy] & PF_ASIAskServerIfModifiedCachePolicy) {
