...
#import "PF_ASIHTTPRequest.h"

#if TARGET_OS_IPHONE
#import "PF_Reachability.h"
#import <MobileCoreServices/MobileCoreServices.h>
#else
#import <SystemConfiguration/SystemConfiguration.h>
#endif
#import "PF_ASIInputStream.h"
#import "PF_ASIDataDecompressor.h"
#import "PF_ASIDataCompressor.h"

// Automatically set on build
NSString *PF_ASIHTTPRequestVersion = @"v1.8.1-8 2011-06-05";

static NSString *defaultUserAgent = nil;

NSString* const PF_NetworkRequestErrorDomain = @"ASIHTTPRequestErrorDomain";

static NSString *PF_ASIHTTPRequestRunLoopMode = @"ASIHTTPRequestRunLoopMode";
...
const int PF_RedirectionLimit = 5;
...
static void ReadStreamClientCallBack(CFReadStreamRef readStream, CFStreamEventType type, void *clientCallBackInfo) {
    [((PF_ASIHTTPRequest*)clientCallBackInfo) handleNetworkEvent: type];
}
...
unsigned long const PF_ASIWWANBandwidthThrottleAmount = 14800;
...
static id <PF_ASICacheDelegate> defaultCache = nil;
...
- (void)fetchPACFile;
- (void)finishedDownloadingPACFile:(PF_ASIHTTPRequest *)theRequest;
...
- (void)performBlockOnMainThread:(PF_ASIBasicBlock)block;
- (void)releaseBlocksOnMainThread;
+ (void)releaseBlocks:(NSArray *)blocks;
- (void)callBlock:(PF_ASIBasicBlock)block;
#endif
...
@property (assign) PF_ASIAuthenticationState authenticationNeeded;
...
@property (retain, nonatomic) PF_ASIHTTPRequest *PACFileRequest;
...
@implementation PF_ASIHTTPRequest

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [PF_ASIHTTPRequest class]) {
		...
		ASIRequestTimedOutError = [[NSError alloc] initWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request timed out",NSLocalizedDescriptionKey,nil]];  
		ASIAuthenticationError = [[NSError alloc] initWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Authentication needed",NSLocalizedDescriptionKey,nil]];
		ASIRequestCancelledError = [[NSError alloc] initWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request was cancelled",NSLocalizedDescriptionKey,nil]];
		ASIUnableToCreateRequestError = [[NSError alloc] initWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create request (bad url?)",NSLocalizedDescriptionKey,nil]];
		ASITooMuchRedirectionError = [[NSError alloc] initWithDomain:PF_NetworkRequestErrorDomain code:PF_ASITooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request failed because it redirected too many times",NSLocalizedDescriptionKey,nil]];
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setTimeOutSeconds:[PF_ASIHTTPRequest defaultTimeOutSeconds]];
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <PF_ASICacheDelegate>)cache
{
	return [self requestWithURL:newURL usingCache:cache andCachePolicy:PF_ASIUseDefaultCachePolicy];
}

+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <PF_ASICacheDelegate>)cache andCachePolicy:(PF_ASICachePolicy)policy
{
	PF_ASIHTTPRequest *request = [[[self alloc] initWithURL:newURL] autorelease];
...
- (void)dealloc
{
	[self setAuthenticationNeeded:PF_ASINoAuthenticationNeededYet];
...
- (void)buildPostBody
{
	...
	if ([self postBodyFilePath]) {
		...
		if ([self shouldCompressRequestBody]) {
			if (![self compressedPostBodyFilePath]) {
				...
				NSError *err = nil;
				if (![PF_ASIDataCompressor compressDataFromFile:[self postBodyFilePath] toFile:[self compressedPostBodyFilePath] error:&err]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",path],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	...
	// Otherwise, we have an in-memory request body
		...
		if ([self shouldCompressRequestBody]) {
			NSError *err = nil;
			NSData *compressedBody = [PF_ASIDataCompressor compressData:[self postBody] error:&err];
			if (err) {
				[self failWithError:err];
				return;
			}
...
{	
	if ([self isResponseCompressed] && [self shouldWaitToInflateCompressedResponses]) {
		return [PF_ASIDataDecompressor uncompressData:[self rawResponseData] error:NULL];
	} else {
...
- (void)startSynchronous
{
	...
	[self setRunLoopMode:PF_ASIHTTPRequestRunLoopMode];
...
- (void)main
{
	@try {
		...
		if ([PF_ASIHTTPRequest isMultitaskingSupported] && [self shouldContinueWhenAppEntersBackground]) {
		...
		if ([self downloadCache]) {
			...
			if ([self cachePolicy] & (PF_ASIAskServerIfModifiedWhenStaleCachePolicy|PF_ASIAskServerIfModifiedCachePolicy)) {
		...
		NSError *underlyingError = [NSError errorWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIUnhandledExceptionError userInfo:[exception userInfo]];
		[self failWithError:[NSError errorWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIUnhandledExceptionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[exception name],NSLocalizedDescriptionKey,[exception reason],NSLocalizedFailureReasonErrorKey,underlyingError,NSUnderlyingErrorKey,nil]]];
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (!userAgentString) {
			userAgentString = [PF_ASIHTTPRequest defaultUserAgentString];
		}
		if (userAgentString) {
...
- (void)updatePartialDownloadSize
{
	...
	if ([self allowResumeForFileDownloads] && [self downloadDestinationPath] && [self temporaryFileDownloadPath] && [fileManager fileExistsAtPath:[self temporaryFileDownloadPath]]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",[self temporaryFileDownloadPath]],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	}
}
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self compressedPostBodyFilePath] && [fileManager fileExistsAtPath:[self compressedPostBodyFilePath]]) {
			[self setPostBodyReadStream:[PF_ASIInputStream inputStreamWithFileAtPath:[self compressedPostBodyFilePath] request:self]];
		} else {
			[self setPostBodyReadStream:[PF_ASIInputStream inputStreamWithFileAtPath:[self postBodyFilePath] request:self]];
		}
		...
		if ([self postBody] && [[self postBody] length] > 0) {
			if ([self shouldCompressRequestBody] && [self compressedPostBody]) {
				[self setPostBodyReadStream:[PF_ASIInputStream inputStreamWithData:[self compressedPostBody] request:self]];
			} else if ([self postBody]) {
				[self setPostBodyReadStream:[PF_ASIInputStream inputStreamWithData:[self postBody] request:self]];
			}
			[self setReadStream:[(NSInputStream *)CFReadStreamCreateForStreamedHTTPRequest(kCFAllocatorDefault, request,(CFReadStreamRef)[self postBodyReadStream]) autorelease]];
	...
	if (![self readStream]) {
		[self failWithError:[NSError errorWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create read stream",NSLocalizedDescriptionKey,nil]]];
        return;
    }
	...
	[PF_ASIHTTPRequest expirePersistentConnections];
   ...
   // Start the HTTP connection
	...
	if (!streamSuccessfullyOpened) {
		...
		[self failWithError:[NSError errorWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to start HTTP connection",NSLocalizedDescriptionKey,nil]]];
		return;	
	}
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
			...
			[PF_ASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:1];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
			[PF_ASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:0 ofTotal:1];
		}
	}	
...
- (void)performRedirect
{
	...
	if ([self redirectCount] > PF_RedirectionLimit) {
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				[self setLastActivityTime:[NSDate date]];
				[PF_ASIHTTPRequest incrementBandwidthUsedInLastSecond:(unsigned long)(totalBytesSent-lastBytesSent)];		
...
- (PF_ASIHTTPRequest *)HEADRequest
{
	PF_ASIHTTPRequest *headRequest = [[self class] requestWithURL:[self url]];
...
- (void)setUploadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[PF_ASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&uploadProgressDelegate withObject:nil amount:&max callerToRetain:nil];
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)setDownloadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[PF_ASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&downloadProgressDelegate withObject:nil amount:&max callerToRetain:nil];	
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)updateDownloadProgress
{
	...
	[PF_ASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[PF_ASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegate withObject:self amount:&value callerToRetain:self];

	[PF_ASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
	...
	[PF_ASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[PF_ASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&value callerToRetain:self];
	[PF_ASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
...
{
	[PF_ASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[PF_ASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
{
	[PF_ASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[PF_ASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
-(void)removeUploadProgressSoFar
{
	...
	[PF_ASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&progressToRemove callerToRetain:self];
	[PF_ASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&progressToRemove callerToRetain:self];
	[PF_ASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:[self postLength]];
...
+ (void)performSelector:(SEL)selector onTarget:(id *)target withObject:(id)object amount:(void *)amount callerToRetain:(id)callerToRetain
{
	if ([*target respondsToSelector:selector]) {
        ...
        NSMethodSignature *cbSignature = [PF_ASIHTTPRequest methodSignatureForSelector:callback];
...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	...
	[progressLock lock];
	[PF_ASIHTTPRequest performSelector:selector onTarget:indicator withObject:nil amount:&progressAmount callerToRetain:nil];
	[progressLock unlock];
}
...
- (void)failWithError:(NSError *)theError
{
	...
	if (theError && [theError code] != PF_ASIAuthenticationErrorType && [theError code] != PF_ASITooMuchRedirectionErrorType) {
		[connectionsLock lock];
		#if DEBUG_PERSISTENT_CONNECTIONS
	...
	// If we have cached data, use it and ignore the error when using PF_ASIFallbackToCacheIfLoadFailsCachePolicy
	if ([self downloadCache] && ([self cachePolicy] & PF_ASIFallbackToCacheIfLoadFailsCachePolicy)) {
		if ([[self downloadCache] canUseCachedDataForRequest:self]) {
			[self useDataFromCache];
			return;
		}
	...
	PF_ASIHTTPRequest *failedRequest = self;
...
- (void)readResponseHeaders
{
	...
	if ([self responseStatusCode] == 401) {
		[self setAuthenticationNeeded:PF_ASIHTTPAuthenticationNeeded];
	} else if ([self responseStatusCode] == 407) {
		[self setAuthenticationNeeded:PF_ASIProxyAuthenticationNeeded];
	} else {
		#if DEBUG_HTTP_AUTHENTICATION
	...
	if ([self useCookiePersistence]) {
		...
		for (cookie in newCookies) {
			[PF_ASIHTTPRequest addSessionCookie:cookie];
		}
	}
	...
	if (![self willRedirect]) {
		...
		PF_ASIHTTPRequest *theRequest = self;
		if ([self mainRequest]) {
...
- (BOOL)willRedirect
{
	...
	// Note that PF_ASIHTTPRequest does not currently support 305 Use Proxy
...
- (void)saveProxyCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[PF_ASIHTTPRequest saveCredentials:authenticationCredentials forProxy:[self proxyHost] port:[self proxyPort] realm:[self proxyAuthenticationRealm]];
	}	
}
...
- (void)saveCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[PF_ASIHTTPRequest saveCredentials:authenticationCredentials forHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
	}	
}
...
- (NSMutableDictionary *)findProxyCredentials
{
	...
	PF_ASIHTTPRequest *theRequest = [self mainRequest];
	...
	if ((!user || !pass)) {
		NSURLCredential *authenticationCredentials = [PF_ASIHTTPRequest savedCredentialsForProxy:[self proxyHost] port:[self proxyPort] protocol:[[self url] scheme] realm:[self proxyAuthenticationRealm]];
		if (authenticationCredentials) {
...
- (NSMutableDictionary *)findCredentials
{
	...
	if ((!user || !pass) && useKeychainPersistence) {
		NSURLCredential *authenticationCredentials = [PF_ASIHTTPRequest savedCredentialsForHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
		if (authenticationCredentials) {
			user = [authenticationCredentials user];
			pass = [authenticationCredentials password];
			#if DEBUG_HTTP_AUTHENTICATION
...
{
    // NOTE(ilya): Did this to rip out the authentication dialog code.
    return NO;
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to get authentication object from response headers",NSLocalizedDescriptionKey,nil]]];
		return;
	}
	...
	if (proxyCredentials) {
		...
		// Something went wrong, we'll have to give up
			...
			[self failWithError:[NSError errorWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply proxy credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if ([self authenticationNeeded] == PF_ASIProxyAuthenticationNeeded) {
		[self attemptToApplyProxyCredentialsAndResume];
		return;
	}
	...
	if (requestCredentials) {
		...
		if ((([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM) || [self authenticationRetryCount] < 2) && [self applyCredentials:requestCredentials]) {
			...
			[self failWithError:[NSError errorWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
{
	[self addRequestHeader:@"Authorization" value:[NSString stringWithFormat:@"Basic %@",[PF_ASIHTTPRequest base64forData:[[NSString stringWithFormat:@"%@:%@",theUsername,thePassword] dataUsingEncoding:NSUTF8StringEncoding]]]];	
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if ([self isResponseCompressed] && ![self shouldWaitToInflateCompressedResponses]) {
			if (![self dataDecompressor]) {
				[self setDataDecompressor:[PF_ASIDataDecompressor decompressor]];
			}
			NSError *err = nil;
		...
		[PF_ASIHTTPRequest incrementBandwidthUsedInLastSecond:bytesRead];
	...
	if ([self temporaryFileDownloadPath]) {
		...
		if ([self shouldRedirect] && [self needsRedirect] && [self allowResumeForFileDownloads]) {
			...
			if ([self shouldWaitToInflateCompressedResponses]) {
				[PF_ASIDataDecompressor uncompressDataFromFile:[self temporaryFileDownloadPath] toFile:[self downloadDestinationPath] error:&fileError];
			...
			// Response should already have been inflated, move the temporary file to the destination path
				...
				if (moveError) {
					fileError = [NSError errorWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to move file from '%@' to '%@'",[self temporaryFileDownloadPath],[self downloadDestinationPath]],NSLocalizedDescriptionKey,moveError,NSUnderlyingErrorKey,nil]];
				}
...
- (void)useDataFromCache
{
	...
	if (headers && dataPath) {
		...
		[self setResponseStatusCode:[[headers objectForKey:@"X-PF_ASIHTTPRequest-Response-Status-Code"] intValue]];
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIConnectionFailureErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:reason,NSLocalizedDescriptionKey,underlyingError,NSUnderlyingErrorKey,nil]]];
	} else {
		[self cancelLoad];
	}
	[self checkRequestStatus];
...
+ (BOOL)removeFileAtPath:(NSString *)path error:(NSError **)err
{
	...
	if ([fileManager fileExistsAtPath:path]) {
		...
		if (removeError) {
			if (err) {
				*err = [NSError errorWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to delete file at path '%@'",path],NSLocalizedDescriptionKey,removeError,NSUnderlyingErrorKey,nil]];
			}
			return NO;
		}
	}
...
- (BOOL)configureProxies
{
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if (!proxies) {
			[self setReadStream:nil];
			[self failWithError:[NSError errorWithDomain:PF_NetworkRequestErrorDomain code:PF_ASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to obtain information on proxy servers needed for request",NSLocalizedDescriptionKey,nil]]];
			return NO;
		}
...
- (void)fetchPACFile
{
	...
	PF_ASIHTTPRequest *PACRequest = [PF_ASIHTTPRequest requestWithURL:[self PACurl]];
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	PF_ASIHTTPRequest *newRequest = [[[self class] alloc] initWithURL:[self url]];
...
+ (NSMutableArray *)sessionCookies
{
	...
	if (!sessionCookies) {
		[PF_ASIHTTPRequest setSessionCookies:[NSMutableArray array]];
	}
...
+ (void)addSessionCookie:(NSHTTPCookie *)newCookie
{
	...
	NSHTTPCookie *cookie;
	NSUInteger i;
	NSUInteger max = [[PF_ASIHTTPRequest sessionCookies] count];
	for (i=0; i<max; i++) {
		cookie = [[PF_ASIHTTPRequest sessionCookies] objectAtIndex:i];
		if ([[cookie domain] isEqualToString:[newCookie domain]] && [[cookie path] isEqualToString:[newCookie path]] && [[cookie name] isEqualToString:[newCookie name]]) {
			[[PF_ASIHTTPRequest sessionCookies] removeObjectAtIndex:i];
			break;
		}
	}
	[[PF_ASIHTTPRequest sessionCookies] addObject:newCookie];
	[sessionCookiesLock unlock];
}
...
+ (void)clearSession
{
	...
	[[[self class] defaultCache] clearCachedResponsesForStoragePolicy:PF_ASICacheForSessionDurationCacheStoragePolicy];
}
...
+ (NSString *)defaultUserAgentString
{
	@synchronized (self) {
		...
		if (!defaultUserAgent) {
			...
			// If we couldn't find one, we'll give up (and PF_ASIHTTPRequest will use the standard CFNetwork user agent)
			if (!appName) {
				return nil;
			}
			...
			NSString *locale = [[NSLocale currentLocale] localeIdentifier];
				...
				// We won't bother to check for systems prior to 10.4, since PF_ASIHTTPRequest only works on 10.5+
				OSErr err;
...
- (void)performThrottling
{
	...
	[PF_ASIHTTPRequest measureBandwidthUsage];
	if ([PF_ASIHTTPRequest isBandwidthThrottled]) {
		[bandwidthThrottlingLock lock];
		// Handle throttling
...
+ (void)measureBandwidthUsage
{
	...
	if (!bandwidthMeasurementDate || [bandwidthMeasurementDate timeIntervalSinceNow] < -0) {
		[PF_ASIHTTPRequest recordBandwidthUsage];
	}
...
{
	if (throttle) {
		[PF_ASIHTTPRequest throttleBandwidthForWWANUsingLimit:PF_ASIWWANBandwidthThrottleAmount];
	} else {
		[PF_ASIHTTPRequest unsubscribeFromNetworkReachabilityNotifications];
		[PF_ASIHTTPRequest setMaxBandwidthPerSecond:0];
		[bandwidthThrottlingLock lock];
...
{	
	...
	[PF_ASIHTTPRequest registerForNetworkReachabilityNotifications];	
	[bandwidthThrottlingLock unlock];
	[PF_ASIHTTPRequest reachabilityChanged:nil];
}
...
+ (void)registerForNetworkReachabilityNotifications
{
	[[PF_Reachability reachabilityForInternetConnection] startNotifier];
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reachabilityChanged:) name:PF_kReachabilityChangedNotification object:nil];
}


+ (void)unsubscribeFromNetworkReachabilityNotifications
{
	[[NSNotificationCenter defaultCenter] removeObserver:self name:PF_kReachabilityChangedNotification object:nil];
}

+ (BOOL)isNetworkReachableViaWWAN
{
	return ([[PF_Reachability reachabilityForInternetConnection] currentReachabilityStatus] == PF_ReachableViaWWAN);	
}

+ (void)reachabilityChanged:(NSNotification *)note
{
	[bandwidthThrottlingLock lock];
	isBandwidthThrottled = [PF_ASIHTTPRequest isNetworkReachableViaWWAN];
	[bandwidthThrottlingLock unlock];
}
#endif
...
+ (void)setDefaultCache:(id <PF_ASICacheDelegate>)cache
{
	@synchronized (self) {
		[cache retain];
...
+ (id <PF_ASICacheDelegate>)defaultCache
{
    @synchronized(self) {
...
+ (NSThread *)threadForRequest:(PF_ASIHTTPRequest *)request
{
...
#pragma mark blocks
#if NS_BLOCKS_AVAILABLE
- (void)setStartedBlock:(PF_ASIBasicBlock)aStartedBlock
{
	[startedBlock release];
	startedBlock = [aStartedBlock copy];
}

- (void)setHeadersReceivedBlock:(PF_ASIHeadersBlock)aReceivedBlock
{
	[headersReceivedBlock release];
	headersReceivedBlock = [aReceivedBlock copy];
}

- (void)setCompletionBlock:(PF_ASIBasicBlock)aCompletionBlock
{
	[completionBlock release];
	completionBlock = [aCompletionBlock copy];
}

- (void)setFailedBlock:(PF_ASIBasicBlock)aFailedBlock
{
	[failureBlock release];
	failureBlock = [aFailedBlock copy];
}

- (void)setBytesReceivedBlock:(PF_ASIProgressBlock)aBytesReceivedBlock
{
	[bytesReceivedBlock release];
	bytesReceivedBlock = [aBytesReceivedBlock copy];
}

- (void)setBytesSentBlock:(PF_ASIProgressBlock)aBytesSentBlock
{
	[bytesSentBlock release];
	bytesSentBlock = [aBytesSentBlock copy];
}

- (void)setDownloadSizeIncrementedBlock:(PF_ASISizeBlock)aDownloadSizeIncrementedBlock{
	[downloadSizeIncrementedBlock release];
	downloadSizeIncrementedBlock = [aDownloadSizeIncrementedBlock copy];
}

- (void)setUploadSizeIncrementedBlock:(PF_ASISizeBlock)anUploadSizeIncrementedBlock
{
	[uploadSizeIncrementedBlock release];
	uploadSizeIncrementedBlock = [anUploadSizeIncrementedBlock copy];
}

- (void)setDataReceivedBlock:(PF_ASIDataBlock)aReceivedBlock
{
	[dataReceivedBlock release];
	dataReceivedBlock = [aReceivedBlock copy];
}

- (void)setAuthenticationNeededBlock:(PF_ASIBasicBlock)anAuthenticationBlock
{
	[authenticationNeededBlock release];
	authenticationNeededBlock = [anAuthenticationBlock copy];
}
- (void)setProxyAuthenticationNeededBlock:(PF_ASIBasicBlock)aProxyAuthenticationBlock
{
	[proxyAuthenticationNeededBlock release];
	proxyAuthenticationNeededBlock = [aProxyAuthenticationBlock copy];
}
- (void)setRequestRedirectedBlock:(PF_ASIBasicBlock)aRedirectBlock
{
